const rangeSliderInit = () => {
  ...
  Array.from(sliders).forEach((slider, i) => {
    ...
    const getCircleWidth = () => circle.getBoundingClientRect().width,
      ...
      getStepWidth = () =>
        (getSliderWidth() - getCircleWidth()) / (steps.length - 1);
    ...
    function updateCurrentStepIndex(clientX) {
      currentStepIndex = Math.round(
        (clientX - getSliderLeftX() - getCircleWidth() / 2) / getStepWidth()
      );
    ...
    function render(force = false) {
      ...
      leftCloud.style.visibility =
        currentStepIndex === 0 ? 'hidden' : 'visible';

      rightCloud.style.visibility =
        currentStepIndex === steps.length - 1 ? 'hidden' : 'visible';

      movingCloud.innerHTML = '' + steps[currentStepIndex];
      slider.dataset.value = '' + steps[currentStepIndex];

      movingCloud.style.left =
        -(
          getCircleBorderWidth() +
          getMovingCloudWidth() / 2 -
          getCircleWidth() / 2
        ) + 'px';
...
export default rangeSliderInit;
